...
async function getTopClans(apiToken) {
  const response = await fetch(
    "https://cocproxy.royaleapi.dev/v1/locations/global/rankings/clans?limit=10",
    {
      headers: {
    ...
    if (existingUser) {
      if (existingUser.email.toLowerCase() === req.body.email.toLowerCase()) {
        return res.render("register", {
          error: "Email bestaat al. Probeer een ander e-mailadres.",
        });
      } else if (
        ...
        return res.render("register", {
          error2:
            "Gebruikersnaam bestaat al. Probeer een andere gebruikersnaam.",
        });
      }
    }
...
app.post("/login", async (req, res, next) => {
  try {
    ...
    const wachtwoordMatch = await bcrypt.compare(
      req.body.password,
      user.password
    );
    ...
    if (wachtwoordMatch) {
      ...
      res.render("login", {
        error: null,
        passwordError: "Ongeldig wachtwoord",
        username: req.session.user,
        profilePhoto,
      });
    }
...
app.post("/auth/reset-wachtwoord/:email", async (req, res) => {
  ...
  const { nieuwwachtwoord: wachtwoord, bevestigwachtwoord: wachtwoord2 } =
    req.body;
  const profilePhoto = await getUserProfilePhoto(req.session.user);

  try {
    if (wachtwoord !== wachtwoord2) {
      return res.render("wachtwoord-reset", {
        email,
        error: "Wachtwoorden komen niet overeen.",
        username: req.session.user,
        profilePhoto,
...
app.get("/search", isAuthenticated, async (req, res) => {
  ...
  try {
    const response = await fetch(
      `https://cocproxy.royaleapi.dev/v1/clans?name=${encodeURIComponent(
        query
      )}`,
      {
        headers: {
...
app.get("/clan/:clanTag", async (req, res) => {
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(`Clan data ophalen mislukt: ${response.status}`);

    const data = await response.json();
    const membersString = data.memberList
...
app.post("/saveClan", async (req, res) => {
  ...
  try {
    await client
      .db(process.env.DB_NAME)
      .collection("submissions")
      .updateOne({ username }, { $addToSet: { favoriteClans: clanTag } });
...
app.post("/removeClan", async (req, res) => {
  ...
  try {
    await client
      .db(process.env.DB_NAME)
      .collection("submissions")
      .updateOne({ username }, { $pull: { favoriteClans: clanTag } });
...
app.post("/updateProfilePhoto", isAuthenticated, async (req, res) => {
  ...
  try {
    const result = await client
      .db(process.env.DB_NAME)
      .collection("submissions")
      .updateOne({ username }, { $set: { profilePhoto: photoPath } });
    ...
    if (result.modifiedCount === 1) {
      ...
      return res.json({
        success: false,
        message: "Geen gebruiker gevonden om te updaten",
      });
    }
  } catch (err) {
    ...
    return res.status(500).json({
      success: false,
      message: "Database fout bij het opslaan van de profielfoto",
    });
  }
});
...
app.get("/filtered-search", async (req, res) => {
  ...
  try {
    ...
    const townHallLevel = parseInt(
      answers["question-1"]?.["townhall-level"] || 11
    );
    ...
    const response = await fetch(
      `https://cocproxy.royaleapi.dev/v1/clans?${params.toString()}`,
      {
        headers: {
          Authorization: `Bearer ${process.env.COC_API_KEY}`,
        },
    ...
    const filteredClans = clans.filter((clan) => {
      ...
      const locationMatch =
        !countryCode ||
        clan.location?.id?.toString() === countryCode.toString();
...
app.get("/api/locations", async (req, res) => {
  try {
    const response = await fetch(
      "https://cocproxy.royaleapi.dev/v1/locations",
      {
        headers: {
          Authorization: `Bearer ${process.env.COC_API_KEY}`,
        },
...
app.use(async (req, res) => {
  ...
  res
    .status(404)
    .render("404", { url: req.url, username: req.session.user, profilePhoto });
});
